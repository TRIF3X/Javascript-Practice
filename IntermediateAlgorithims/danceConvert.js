...
function danceConvert(nums) {
    const moves = 
      ["Shimmy", "Shake", "Pirouette", "Slide", "Box Step", 
       "Headspin", "Dosado", "Pop", "Lock", "Arabesque"];
       //if a given num contains a letter return
    if(nums.match(/[a-z]/ig)) {
        return 'invalid input'
        //split the nums into strings, map over the strings, 
    } else {
        return nums.split('').map((v,k) => moves[(Number(v) + k) % moves.length])
    }
